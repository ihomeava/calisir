...
%>
	<%-- add by weicf 2012.01.12 --%>
	<suggest:itemSuggestsStatTag resourceID="<%= item.getID() %>"/>
    <a href="<%= request.getContextPath() %>/suggest?handle=<%= handle %>&resourceID=<%= item.getID() %>" target="new_window">
...
<%
        ...
        {
%>
	&nbsp;&nbsp;&nbsp;&nbsp;<a href="<%= request.getContextPath() %>/favorite?handle=<%= handle %>&resourceID=<%= item.getID() %>" target="new_window"><fmt:message key="jsp.display-item.favorite"/></a>
<%
        }
     	// add by weicf
%>
...
%>
	...
	<commenting:commentTag 
		callerUrl="<%= locationLink %>"
		resourceType="<%= Constants.ITEM %>"
		resourceID="<%= item.getID() %>"
		autoApproveAnonymousComments="false"/>
    <%-- add by weicf --%>		
