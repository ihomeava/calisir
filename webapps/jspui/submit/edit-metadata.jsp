...
<%@ page import="edu.calis.ir.bit.web.util.FormTpye" %>
...
//             .append("Report or Paper No.</td>")
      ...
      throws java.io.IOException
    {
      ...
      for (int j = 0; j < fieldCount; j++)
      {
         ...
         // do the dropdown box
         sb.append("<td colspan=\"2\" align=\"left\"><select name=\"")
           .append(fieldName)
           .append("_qualifier");
    ...
    void doList(javax.servlet.jsp.JspWriter out, Item item,
            ...
            throws java.io.IOException
          {
            ...
            if(numColumns > 1)
                ...
                sb.append("<td valign=\"top\" colspan=\"3\"  align=\"left\">");
...
<%
    ...
    // add by weicf 2012.04.18
    String formName = inputSet.getFormName();
    String typeSelect = "";
    if(item.getMetadata("dc.type") != null && item.getMetadata("dc.type").length > 0){
    	typeSelect = item.getMetadata("dc.type")[0].value;
    } else {
    	typeSelect = FormTpye.getType(formName);
    }
    // add by weicf
    Integer pageNumStr =
...
%>
     ...
     <center>
       ...
       <%-- add by weicf 2012.02.13 --%>
     <script>
     function findDuplicate(){
	     var author = document.edit_metadata.dc_contributor_author_last_1.value + document.edit_metadata.dc_contributor_author_first_1.value;
	     var title = $$('#dc_title')[0].value;
	     if(author == "" || title == "") {
	    	 alert('<fmt:message key="jsp.submit.edit-metadata.duplicate.msg" />'); 
	    	 return false;
	     }
	     var cOffset = 0;
	     ...
	     if (left < 0) left = 0;
	     if (top < 0) top = 0;     
	     var popWin = window.open("<%=request.getContextPath() %>/simple-search?query=%28%28author%3A" + encodeURI(author) + "%29%20AND%20%28" + encodeURI(title) + "%29%29&from_advanced=true&conjunction2=AND&field3=ANY&field2=title&conjunction1=AND&field1=author&query1=" + escape(author) + "&query2=" + escape(title) + "&query3=", 
	    		 'ignoreme',
	    	     'width='+width+
	    	     ',height='+height+
	    	     ',left='+left+
     ...
     <center>
     <div><img src="<%=request.getContextPath()%>/image/zoom.png"/> <a href="javascript:void(null);" onclick='findDuplicate()'><fmt:message key="jsp.submit.edit-metadata.duplicate"/></a></div>
     <%-- add by weicf --%>
...
<%
		...
		if("language".equals(dcElement) && (z + 1) < inputs.length){
			ifPrintMore = true;
%>
		</table>
		<table>
		   <tr>
	 	   ...
	 	   <tr>
	         <td width="25%"></td>
	         <td colspan="2" width="65%"><a href="javascript:void(0);" onclick="javascript:$$('#morefileds')[0].style.display =($$('#morefileds')[0].style.display=='none')?'block':'none';"><fmt:message key="jsp.submit.initial-questions.more"/></a></td>
	         <td width="10%">&nbsp;</td>
	       </tr>
	    </table>
		<table  id="morefileds" style="display:none">
<%
		}
%>
       <tr>
...
<script type="text/javascript">
	var dcTypes = document.getElementsByName("dc_type");
	for (var i = 0; i < dcTypes.length; i++) {
		if (dcTypes[i].value == "<%=typeSelect %>") {
			dcTypes[i].checked = true
			break;
		}
	}
</script>