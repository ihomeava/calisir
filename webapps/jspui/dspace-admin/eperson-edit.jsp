...
<%  } %>
	...
	function checkForm() {
		if(!checkPassword()) {
			alert('<fmt:message key="jsp.register.edit-profile.info2"/>');
			return false;
		}
		if (!checkRePassword()) {
			alert('<fmt:message key="jsp.dspace-admin.eperson-edit.password-valid-error"/>');
			return false;
		}
		return true;
	}
	...
	function checkPassword() {
		if ($F("tpassword").match(/^\s*$/g)) {
			return false;
		} else if ($F("tpassword").length < 6) {
			return false;
		} else {
			return true;
		}
	...
	function checkRePassword(){
	    if ($F("tpasswordValid").match(/^\s*$/g)) {
	        return false;
	    } else if($F("tpasswordValid")!=$F("tpassword")){
	   		return false; 
	    } else {
	      	return true;
	    }
...
%>
        ...
        <%-- add by weic 2011.09.05 --%>
        <%
        if (ldap_enabled == false && AuthorizeManager.isAdmin(context))
        {
        	%>
        <tr>
        	<th colspan="2">
        		<hr />
        		<fmt:message key="jsp.dspace-admin.eperson-edit.password-note"/>
        	</th>
        </tr>
        <tr>
            <td><label for="tpassword"><fmt:message key="jsp.dspace-admin.eperson-edit.password"/></label></td>
            <td>
                <input name="password" type="password" id="tpassword" size="24" value="" />
            </td>
        </tr>
        <tr>
            <td><label for="tpasswordValid"><fmt:message key="jsp.dspace-admin.eperson-edit.password-valid"/></label></td>
            <td>
                <input id="tpasswordValid" type="password" size="24" value="" />
            </td>
        </tr>
        	<%
        }	
